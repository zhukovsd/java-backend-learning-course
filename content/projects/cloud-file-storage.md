...
Ответ в случае успеха: `200 OK` со следующим телом (`application/json`):
...
Ответ в случае успеха: `200 OK` со следующим телом (`application/json`). Коллекция найденных ресурсов:
...
Ответ в случае успеха: `201 Created` со следующим телом (`application/json`). Коллекция загруженных ресурсов:
...
Ответ в случае успеха: `201 Created` со следующим телом (`application/json`). Ресурс созданной папки:
...
Предлагаю реализовать работу с сессиями и куками вручную, без Spring Security. Пользователей храним в SQL БД, сессии - в Redis.

Альтернативно, если хотите получить от роадмапа больше опыта со Spring Security (который сейчас является обязательным только в последнем проекте), то вы можете не писать работу с сессиями руками, а воспользоваться экосистемой Spring - Security и Session. 

В таком случае, за работу с сессиями будет отвечатьSpring Sessions. По умолчанию Spring Boot хранит сессии внутри приложения, и они теряются после каждого перезапуска приложения. Поэтому, можем воспользоваться Redis. Пример - [https://www.baeldung.com/spring-session](https://www.baeldung.com/spring-session). Redis - NoSQL хранилище, имеющее встроенный TTL (time to live) атрибут для записей, что делает его удобным для хранения сессий - истекшие записи автоматически удаляются. 
...
- Для работы с протоколом S3 воспользуемся [Minio Java SDK](https://docs.min.io/aistor/developers/sdk/java/)
