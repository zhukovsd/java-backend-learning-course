...
Для каждого полученного сообщения, содержимое которого десериализуется в экземпляр модели, пользуемся модулем Spring Mail, чтобы отправить письмо. Подробнее о том, откуда брать SMTP конфигурацию, написано ниже в разделе "Сервис для отправки писем".
...
Выбор того или иного варианта зависит от ситуации. В рабочих проектах вариант #1 встречается чуть чаще. Вариант #2 сложнее в случае использования многомодульного Maven/Gradle проекта.
...
С помощью GitHub Actions автоматизируем сборку докер образов для сервисов приложения (task-tracker-*):
